...
if nxfs.access('/etc/config/argon_new') then
	primary = uci:get_first('argon_new', 'global', 'primary')
	dark_primary = uci:get_first('argon_new', 'global', 'dark_primary')
	blur_radius = uci:get_first('argon_new', 'global', 'blur')
	blur_radius_dark = uci:get_first('argon_new', 'global', 'blur_dark')
	blur_opacity = uci:get_first('argon_new', 'global', 'transparency')
	blur_opacity_dark = uci:get_first('argon_new', 'global', 'transparency_dark')
	mode = uci:get_first('argon_new', 'global', 'mode')
	bing_background = uci:get_first('argon_new', 'global', 'bing_background')
end
...
-- [[ 模糊设置 ]]--
br = SimpleForm('config', translate('Argon_new Config'), translate('Here you can set the blur and transparency of the login page of argon_new theme, and manage the background pictures and videos.[Chrome is recommended]'))
br.reset = false
br.submit = false
...
function br.handle(self, state, data)
    if (state == FORM_VALID and data.blur ~= nil and data.blur_dark ~= nil and data.transparency ~= nil and data.transparency_dark ~= nil and data.mode ~= nil) then
        ...
        for key, value in pairs(data) do
            uci:set('argon_new','@global[0]',key,value)
        end 
        uci:commit('argon_new')
    end
    return true
end
...
sul = ful:section(SimpleSection, '', translate("Upload file to '/www/luci-static/argon_new/background/'"))
fu = sul:option(FileUpload, '')
fu.template = 'argon_new-config/other_upload'
um = sul:option(DummyValue, '', nil)
um.template = 'argon_new-config/other_dvalue'

local dir, fd
dir = '/www/luci-static/argon_new/background/'
nxfs.mkdir(dir)
http.setfilehandler(
    function(meta, chunk, eof)
        ...
        if eof and fd then
            fd:close()
            fd = nil
            um.value = translate('File saved to') .. ' "/www/luci-static/argon_new/background/' .. meta.file .. '"'
        end
    end
)
